...
class SubdomainHandlingTest(unittest.TestCase):
    ...
    def test_subdomain_fallback_add_record(self):
        ...
        original_method = self.client.__class__.__bases__[0].add_txt_record
        with mock.patch.object(self.client.__class__.__bases__[0], 'add_txt_record', side_effect=side_effect) as mock_add:
            try:
                # Should not raise an exception
                self.client.add_txt_record('my.example.com', '_acme-challenge', 'test-value')
                # Verify it was called multiple times (exact count may vary due to implementation)
    ...
    def test_subdomain_fallback_del_record(self):
        ...
        # Mock the parent class method directly
        with mock.patch.object(self.client.__class__.__bases__[0], 'del_txt_record', side_effect=side_effect) as mock_del:
            try:
                # Should not raise an exception
                self.client.del_txt_record('my.example.com', '_acme-challenge', 'test-value')
                # Verify it was called multiple times (exact count may vary due to implementation)
    ...
    def test_deep_subdomain_fallback(self):
        ...
        def side_effect(domain, record_name, record_content):
            if domain == 'sub.my.example.com':
                ...
                raise Exception("No matching domain found") 
            elif domain == 'example.com' and record_name == '_acme-challenge.sub.my':
                return None  # Success
            else:
                raise Exception(f"Unexpected call: {domain}, {record_name}")
        ...
        with mock.patch.object(self.client.__class__.__bases__[0], 'add_txt_record', side_effect=side_effect) as mock_add:
            try:
                # Should not raise an exception
                self.client.add_txt_record('sub.my.example.com', '_acme-challenge', 'test-value')
                # Verify it was called multiple times (exact count may vary due to implementation)
