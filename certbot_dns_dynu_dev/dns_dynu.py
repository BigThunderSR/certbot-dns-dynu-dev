...
class Authenticator(dns_common.DNSAuthenticator):
    ...
    def _get_dynu_client(self):
        ...
        assert self.credentials is not None, "Credentials should be set after _setup_credentials()"
...
class _DynuLexiconClient(dns_common_lexicon.LexiconClient):
    ...
    def add_txt_record(self, domain, record_name, record_content):
        ...
        try:
            ...
            super(_DynuLexiconClient, self).add_txt_record(domain, record_name, record_content)
            ...
            for i in range(len(domain_parts) - 1):
                ...
                try:
                    logger.debug("Trying to add TXT record to parent zone %s with name %s", 
                               parent_domain, adjusted_name)
                    super(_DynuLexiconClient, self).add_txt_record(parent_domain, adjusted_name, record_content)
                    return  # Success, exit the function
                except Exception as parent_e:
                    logger.debug("Failed to add TXT record to %s zone: %s", parent_domain, parent_e)
                    continue
    ...
    def del_txt_record(self, domain, record_name, record_content):
        ...
        try:
            ...
            super(_DynuLexiconClient, self).del_txt_record(domain, record_name, record_content)
        except Exception as e:
            logger.debug("Failed to delete TXT record from %s zone: %s", domain, e)
            # If that fails, try to find the parent domain zone
            # Split the domain and try progressively shorter domains
            domain_parts = domain.split('.')
            for i in range(len(domain_parts) - 1):
                ...
                try:
                    logger.debug("Trying to delete TXT record from parent zone %s with name %s", 
                               parent_domain, adjusted_name)
                    super(_DynuLexiconClient, self).del_txt_record(parent_domain, adjusted_name, record_content)
                    return  # Success, exit the function
                except Exception as parent_e:
                    logger.debug("Failed to delete TXT record from %s zone: %s", parent_domain, parent_e)
                    continue
    ...
    def _handle_general_error(self, e, domain_name):
        ...
        return super(_DynuLexiconClient, self)._handle_general_error(e, domain_name)
