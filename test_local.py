...
from certbot_dns_dynu_dev.dns_dynu import _DynuLexiconClient  # noqa: E402
...
class MockDynuLexiconClient(_DynuLexiconClient):
    ...
    def _mock_parent_add_txt_record(self, domain, record_name,
                                    record_content):
        ...
        print(f"✅ Successfully added TXT record: {record_name} = "
              f"{record_content} in zone {domain}")

    def _mock_parent_del_txt_record(self, domain, record_name,
                                    record_content):
        ...
        print(f"✅ Successfully deleted TXT record: {record_name} = "
              f"{record_content} from zone {domain}")
...
def test_subdomain_scenarios():
    ...
    with mock.patch.object(_DynuLexiconClient.__bases__[0], 'add_txt_record') as mock_add, \
        ...
        print("Expected: Should fail for my.domain.com zone, "
              "succeed for domain.com zone")
        print("─" * 50)
        ...
        try:
            client.add_txt_record('my.domain.com', '_acme-challenge',
        ...
        print("Expected: Should fail for api.my.domain.com and "
              "my.domain.com, succeed for domain.com")
        print("─" * 50)
        ...
        try:
            client.add_txt_record('api.my.domain.com', '_acme-challenge',
                                  'test-validation-token')
            print("Result: ✅ Success")
        except Exception as e:
            print(f"Result: ❌ Failed with: {e}")
        ...
        try:
            client.add_txt_record('domain.com', '_acme-challenge',
                                  'test-validation-token')
            print("Result: ✅ Success")
        except Exception as e:
            print(f"Result: ❌ Failed with: {e}")
        ...
        try:
            client.del_txt_record('my.domain.com', '_acme-challenge',
                                  'test-validation-token')
            print("Result: ✅ Success")
        except Exception as e:
            print(f"Result: ❌ Failed with: {e}")
...
def test_record_name_construction():
    ...
    test_cases = [
        ('my.domain.com', '_acme-challenge', 'domain.com',
         '_acme-challenge.my'),
        ('api.my.domain.com', '_acme-challenge', 'domain.com',
         '_acme-challenge.api.my'),
        ('test.api.my.domain.com', '_acme-challenge', 'domain.com',
         '_acme-challenge.test.api.my'),
        ('domain.com', '_acme-challenge', 'domain.com', '_acme-challenge'),
    ]

    for (original_domain, record_name, target_zone,
         expected_adjusted_name) in test_cases:
...
def create_test_credentials():
    ...
    print("  --authenticator dns-dynu \\")
    print(f"  --dns-dynu-credentials {temp_file} \\")
    print("  -d your-domain.com")
